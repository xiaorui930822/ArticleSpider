...
es = connections.create_connection(ArticalType._doc_type.using)
...
def remove_source(value):
    ...
    value = value.replace("发布于拉勾网","")
    if '天前' in value:
        value = value.replace("天前","")
        days = int(value)*(-1)
        value = today + timedelta(days=days)
        value = str(value)
    else:
        value = str(today)+' '+value
    return value
...
class JobBoleArticalItem(scrapy.Item):
    ...
    def get_insert_sql(self):
        insert_sql = """
                  insert into jobbole_artical(title, url, create_date, url_object_id, front_image_url, praise_nums, comments_nums, fav_nums, tags, content)
    ...
    def save_to_es(self):
        ...
        artical.suggest = gen_suggests(ArticalType._doc_type.index, ((artical.title, 10), (artical.tags, 7), (artical.content, 3)))
        artical.save()
...
class LagouJobItem(scrapy.Item):
    ...
    def get_insert_sql(self):
        insert_sql = """
            ...
            job_type, publish_time,tags, job_advantage, job_desc, job_addr, company_name, company_url,
            crawl_time, craw_update_time) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE salary=VALUES(salary), job_desc=VALUES(job_desc)
        """
        craw_update_time = datetime.now()
        work_years = remove_years_tags(self["work_years"])
        publish_time = remove_source(self["publish_time"])
        job_addr = self["job_addr"]
        params = (
            self["title"], self["url"], self["url_object_id"], self["salary"], self["job_city"],
            work_years, self["degree_need"], self["job_type"],
            publish_time, self["tags"], self["job_advantage"], self["job_desc"],
            self["job_addr"], self["company_name"], self["company_url"],
            self["crawl_time"].strftime(SQL_DATETIME_FORMAT),craw_update_time.strftime(SQL_DATETIME_FORMAT)
        )
        return insert_sql, params
    ...
    def save_to_es(self):
        ...
        artical.work_years = remove_years_tags(self["work_years"])
        artical.degree_need = self["degree_need"]
        artical.job_type = self["job_type"]
        artical.tags = self["tags"]
        artical.publish_time = remove_source(self["publish_time"])
        ...
        artical.suggest = gen_suggests(LagouType._doc_type.index, ((artical.title, 10), (artical.tags, 7),(artical.job_desc, 3)))
        artical.save()
